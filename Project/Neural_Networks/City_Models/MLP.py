...
def plot_model_mape(history, string):
    ...
    plt.savefig('Project/Dashboards/Assets/images/City/' + string + '_MAPE_MLP.png', bbox_inches='tight')
...
def plot_model_loss(history, string):
    ...
    plt.savefig('Project/Dashboards/Assets/images/City/' + string + '_Loss_MLP.png', bbox_inches='tight')

# MLP neural network moudle.

def MLP_Network(data, string):
    ...
    with open("Project/Saved_Models/Cities/" + string + "/MLP/" + string + "_MLP.json", "w") as json_file:
        json_file.write(MLP_JSON)
        
    MLP.save_weights('Project/Saved_Models/Cities/' + string + '/MLP/' + string + '_MLP.h5')
...
NYC_CNN = MLP_Network(NYC, 'NYC')
tf.keras.backend.clear_session()

Pittsburgh_CNN = MLP_Network(Pittsburgh, 'Pittsburgh')
tf.keras.backend.clear_session()

SanDiego_CNN = MLP_Network(SanDiego, 'SanDiego')
tf.keras.backend.clear_session()

SanFran_CNN = MLP_Network(SanFran, 'SanFran')
tf.keras.backend.clear_session()

Vancouver_CNN = MLP_Network(Vancouver, 'Vancouver')
tf.keras.backend.clear_session()