...
def plot_model_mape(history, string):
    ...
    plt.savefig('Project/Dashboards/assets/images/City/' + string + '_MAPE_CNN_LSTM.png', bbox_inches='tight')
...
def plot_model_loss(history, string):
    ...
    plt.savefig('Project/Dashboards/assets/images/City/' + string + '_Loss_CNN_LSTM.png', bbox_inches='tight')

# Convolutional neural network moudle.

def CNN_LSTM_Network(data, string):
    ...
    with open("Project/Saved_Models/Cities/" + string + "/CNN_LSTM/" + string + "_CNN_LSTM.json", "w") as json_file:
        json_file.write(CNN_LSTM_JSON)
        
    CNN_LSTM.save_weights('Project/Saved_Models/Cities/' + string + '/CNN_LSTM/' + string + '_CNN_LSTM.h5')
...
NYC_CNN_LSTM = CNN_LSTM_Network(NYC, 'NYC')
tf.keras.backend.clear_session()

Pittsburgh_CNN_LSTM = CNN_LSTM_Network(Pittsburgh, 'Pittsburgh')
tf.keras.backend.clear_session()

SanDiego_CNN_LSTM = CNN_LSTM_Network(SanDiego, 'SanDiego')
tf.keras.backend.clear_session()

SanFran_CNN_LSTM = CNN_LSTM_Network(SanFran, 'SanFran')
tf.keras.backend.clear_session()

Vancouver_CNN_LSTM = CNN_LSTM_Network(Vancouver, 'Vancouver')
tf.keras.backend.clear_session()