...
def plot_model_mape(history, string):
    ...
    plt.savefig('Project/Dashboards/assets/images/Solar/' + string + '_MAPE_CNN.png', bbox_inches='tight')
...
def plot_model_loss(history, string):
    ...
    plt.savefig('Project/Dashboards/assets/images/Solar/' + string + '_Loss_CNN.png', bbox_inches='tight')
...
def Convultional(data, string):
    ...
    for train_index, test_index in tscv.split(data.scaled_dataset):
        ...
        forecast = CNN.predict(data.X_test)
        CNN_forecast = data.scaler.inverse_transform(forecast)
        rms = sqrt(mean_squared_error(y_test_inv, CNN_forecast)) 
    ...
    with open("Project/Saved_Models/Solar/" + string + "/CNN/" + string + "_CNN.json", "w") as json_file:
        json_file.write(CNN_JSON)
        
    CNN.save_weights('Project/Saved_Models/Solar/' + string + '/CNN/' + string + '_CNN.h5')
...
AU_Solar_CNN = Convultional(AU_Solar, 'AU_Solar')
tf.keras.backend.clear_session()

FR_Solar_CNN = Convultional(FR_Solar, 'FR_Solar')
tf.keras.backend.clear_session()

DK_Solar_CNN = Convultional(DK_Solar, 'DK_Solar')
tf.keras.backend.clear_session()
